import 'dotenv/config';
import OpenAI from 'openai';
import { getOpenAIConfig } from './proxyConfig.js';

const openai = new OpenAI(getOpenAIConfig(process.env.OPENAI_API_KEY));

/**
 * Определяет, является ли сообщение возражением
 * @param {string} message - Текст сообщения от клиента
 * @returns {Promise<boolean>} - true если это возражение
 */
export async function isObjection(message) {
    try {
        const completion = await openai.chat.completions.create({
            model: "gpt-4o",
            messages: [
                {
                    role: "system",
                    content: `Ты - помощник для анализа сообщений клиентов в контексте аренды недвижимости.
                    
                    Твоя задача - определить, является ли сообщение клиента ВОЗРАЖЕНИЕМ.
                    
                    ВОЗРАЖЕНИЯ - это:
                    - Сомнения или вопросы о цене (дорого, много, не по карману, почему так дорого)
                    - Сомнения о комиссии (большая комиссия, высокий процент, много берёте)
                    - Вопросы о качестве услуг (а зачем мне агентство, я сам найду)
                    - Срочность/сроки (нужно быстрее, слишком долго, когда сдадите)
                    - Условия договора (не устраивают условия, хочу изменить договор)
                    - Сравнения с конкурентами (у других дешевле, там лучше условия)
                    - Недоверие (а вы точно сдадите, были проблемы раньше)
                    - Любые другие сомнения или вопросы, требующие аргументации
                    
                    НЕ ВОЗРАЖЕНИЯ - это:
                    - Простые вопросы (какой адрес, когда созвонимся)
                    - Согласие/подтверждение (да, хорошо, согласен)
                    - Отказ без аргументации (нет, не интересует, не сейчас)
                    - Приветствия и благодарности
                    - Просьба о дополнительной информации без сомнений
                    
                    Отвечай ТОЛЬКО 'true' или 'false':
                    - 'true' если это ВОЗРАЖЕНИЕ (требует аргументации)
                    - 'false' если это НЕ ВОЗРАЖЕНИЕ`
                },
                {
                    role: "user",
                    content: `Проанализируй сообщение клиента:\n\n"${message}"\n\nЭто возражение?`
                }
            ],
            temperature: 0.3,
            max_tokens: 10
        });

        const result = completion.choices[0]?.message.content?.trim().toLowerCase();
        return result === 'true' || result?.includes('true');
        
    } catch (error) {
        console.error('❌ Ошибка при определении возражения:', error.message);
        if (error.code === 'ECONNREFUSED' || error.code === 'ETIMEDOUT') {
            console.error('🔌 Проблема с подключением. Проверьте доступность прокси-сервера или отключите прокси (USE_PROXY=false)');
        }
        return false;
    }
}

/**
 * Получает ответ на возражение через RAG (File Search)
 * @param {string} objection - Текст возражения от клиента
 * @param {string} vectorStoreId - ID Vector Store с базой знаний
 * @returns {Promise<string>} - Ответ на возражение
 */
export async function handleObjection(objection, vectorStoreId) {
    try {
        console.log(`🔍 Поиск ответа на возражение: "${objection.substring(0, 50)}..."`);
        
        // Создаём временный ассистент с подключённым File Search
        const assistant = await openai.beta.assistants.create({
            name: "Обработчик возражений",
            instructions: `Ты - профессиональный менеджер по недвижимости компании Capital Mars.
            
            Твоя задача - отвечать на возражения клиентов, используя ТОЛЬКО информацию из базы знаний.
            
            ПРАВИЛА:
            1. Отвечай кратко, максимум 1-2 предложения
            2. Используй уверенный и дружелюбный тон
            3. Не извиняйся без причины
            4. Используй конкретные аргументы из базы знаний
            5. Если в базе нет точного ответа, используй близкий по смыслу
            6. НЕ придумывай факты, используй только базу знаний
            7. Отвечай естественно, как живой менеджер
            8. Не начинай ответ с фраз типа "Согласно базе знаний..." или "В документе указано..."
            9. Дай готовую реплику для отправки клиенту, БЕЗ объяснений, рассуждений и подсказок, как менеджеру вести диалог
            10. Не используй слова "например", "можно сказать", "можно ответить", "стоит", "следует"
            
            Формат ответа: Прямой естественный ответ клиенту без предисловий.`,
            model: "gpt-4o",
            tools: [{ type: "file_search" }],
            tool_resources: {
                file_search: {
                    vector_store_ids: [vectorStoreId]
                }
            },
            temperature: 0.7,
            top_p: 1.0
        });

        // Создаём тред для диалога
        const thread = await openai.beta.threads.create({
            messages: [
                {
                    role: "user",
                    content: `Клиент говорит: "${objection}"\n\nКак мне ответить на это возражение?`
                }
            ]
        });

        // Запускаем ассистента с service_tier: flex
        const run = await openai.beta.threads.runs.create(thread.id, {
            assistant_id: assistant.id,
            truncation_strategy: {
                type: "auto"
            },
            temperature: 0.7,
            top_p: 1.0,
            max_prompt_tokens: 10000,
            max_completion_tokens: 500
        });

        // Ожидаем завершения выполнения
        const completedRun = await waitForRunCompletion(thread.id, run.id);

        // Получаем ответ
        const messages = await openai.beta.threads.messages.list(thread.id);
        const assistantMessage = messages.data.find(msg => msg.role === 'assistant');
        
        if (!assistantMessage) {
            throw new Error('Не удалось получить ответ от ассистента');
        }

        const response = assistantMessage.content[0].text.value;

        // Удаляем временный ассистент
        await openai.beta.assistants.del(assistant.id);

        console.log(`✅ Получен ответ на возражение: "${response.substring(0, 50)}..."`);
        
        return formatResponseForWhatsApp(response);
        
    } catch (error) {
        console.error('❌ Ошибка при обработке возражения:', error.message);
        
        // Возвращаем дефолтный ответ при ошибке
        return 'Понимаю ваши сомнения. Давайте я свяжу вас с нашим специалистом, который подробно ответит на все вопросы.';
    }
}

/**
 * Ожидает завершения выполнения Run в Thread
 * @param {string} threadId - ID треда
 * @param {string} runId - ID выполнения
 * @returns {Promise<Object>} - Завершённый Run
 */
async function waitForRunCompletion(threadId, runId) {
    let attempts = 0;
    const maxAttempts = 60; // Максимум 60 попыток (2 минуты)
    
    while (attempts < maxAttempts) {
        try {
            const run = await openai.beta.threads.runs.retrieve(threadId, runId);
            
            if (run.status === 'completed') {
                return run;
            }
            
            if (run.status === 'failed' || run.status === 'cancelled' || run.status === 'expired') {
                throw new Error(`Run завершился со статусом: ${run.status}`);
            }
            
            // Ожидаем 2 секунды перед следующей проверкой
            await new Promise(resolve => setTimeout(resolve, 2000));
            attempts++;
            
        } catch (error) {
            if (error.message.includes('Run завершился')) {
                throw error;
            }
            console.error('❌ Ошибка при проверке статуса Run:', error.message);
            throw error;
        }
    }
    
    throw new Error('Превышено время ожидания завершения Run');
}

/**
 * Форматирует ответ для отправки в WhatsApp
 * @param {string} response - Исходный ответ от GPT
 * @returns {string} - Отформатированный ответ
 */
export function formatResponseForWhatsApp(response) {
    // Удаляем лишние переносы строк
    let formatted = response.trim();
    
    // Заменяем множественные переносы на двойные
    formatted = formatted.replace(/\n{3,}/g, '\n\n');
    
    // Удаляем markdown форматирование (если есть)
    formatted = formatted.replace(/\*\*/g, '');
    formatted = formatted.replace(/\*/g, '');
    formatted = formatted.replace(/#{1,6}\s/g, '');
    
    // Удаляем мета-словечки и шаблоны рассуждений
    const metaPhrases = [
        /^когда клиент говорит[^\.!?]*[\.!?]\s*/i,
        /^понимаю,? что[^\.!?]*[\.!?]\s*/i,
        /\bнапример[,\s]+/gi,
        /\bможно сказать[,\s]+/gi,
        /\bможно ответить[,\s]+/gi,
        /\bстоит\s+[^\.!?]*[,\.]\s*/gi,
        /\bследует\s+[^\.!?]*[,\.]\s*/gi
    ];
    for (const re of metaPhrases) {
        formatted = formatted.replace(re, '');
    }
    
    // Оставляем не более 2 предложений
    const sentences = formatted.split(/(?<=[\.!?])\s+/).filter(Boolean);
    formatted = sentences.slice(0, 2).join(' ');
    
    // Ограничиваем длину до 500 символов (для WhatsApp)
    if (formatted.length > 500) {
        formatted = formatted.substring(0, 497) + '...';
    }
    
    return formatted;
}

/**
 * Главная функция для обработки сообщения клиента
 * @param {string} message - Сообщение от клиента
 * @param {string} vectorStoreId - ID Vector Store
 * @returns {Promise<string|null>} - Ответ на возражение или null если это не возражение
 */
export async function processMessage(message, vectorStoreId) {
    try {
        // Проверяем, является ли сообщение возражением
        const isObjectionMessage = await isObjection(message);
        
        if (!isObjectionMessage) {
            console.log(`ℹ️ Сообщение не является возражением: "${message.substring(0, 50)}..."`);
            return null;
        }
        
        // Если это возражение, обрабатываем его через RAG
        console.log(`🎯 Обнаружено возражение: "${message.substring(0, 50)}..."`);
        const response = await handleObjection(message, vectorStoreId);
        
        return response;
        
    } catch (error) {
        console.error('❌ Ошибка при обработке сообщения:', error.message);
        return null;
    }
}

